...
class CausalMultiHeadAttention(BaseLayer):
    def __init__(self, d_model, num_heads):
        super().__init__()
        assert d_model % num_heads == 0
        self.d_model = d_model
        self.num_heads = num_heads
        self.d_k = d_model // num_heads
        self.layer_type = "CausalMultiHeadAttention"
        self.params = {"Wq": None, "Wk": None, "Wv": None, "Wo": None,
                       "bq": None, "bk": None, "bv": None, "bo": None}
    ...
    def summary(self):
        return ["CausalMHAttn", self.output_shape[1:], self.get_num_parameters(),
                f"{self.num_heads}h"]


class CrossAttention(BaseLayer):
    def __init__(self, d_model, num_heads):
        super().__init__()
        assert d_model % num_heads == 0
        self.d_model = d_model
        self.num_heads = num_heads
        self.d_k = d_model // num_heads
        self.layer_type = "CrossAttention"
        self.params = {"Wq": None, "Wk": None, "Wv": None, "Wo": None,
                       "bq": None, "bk": None, "bv": None, "bo": None}
        self._context = None
    ...
    def forward(self, inputs, context, training=True):
        self.inputs = inputs
        self._context = context
        N, Lq, D = inputs.shape
        N, Lk, D = context.shape

        self.Q = inputs @ self.params["Wq"] + self.params["bq"]
        self.K = context @ self.params["Wk"] + self.params["bk"]
        self.V = context @ self.params["Wv"] + self.params["bv"]
    ...
    def backward(self, grads, learning_rate):
        N, Lq, D = self.inputs.shape
        _, Lk, _ = self._context.shape
        dparams = {}
        ...
        dparams["dWq"] = self.inputs.reshape(-1, D).T @ dQ.reshape(-1, D) / N
        dparams["dWk"] = self._context.reshape(-1, D).T @ dK.reshape(-1, D) / N
        dparams["dWv"] = self._context.reshape(-1, D).T @ dV.reshape(-1, D) / N
        dparams["dbq"] = np.sum(dQ, axis=(0, 1), keepdims=True)
        dparams["dbk"] = np.sum(dK, axis=(0, 1), keepdims=True)
        dparams["dbv"] = np.sum(dV, axis=(0, 1), keepdims=True)

        self._dcontext = dK @ self.params["Wk"].T + dV @ self.params["Wv"].T
        dinputs = dQ @ self.params["Wq"].T
        if self.trainable:
            self.optimizer.update(self, dparams, learning_rate)
        return dinputs

    def get_context_grads(self):
        return self._dcontext

    def get_num_parameters(self):
        return sum(np.prod(v.shape) for v in self.params.values())

    def summary(self):
        return ["CrossAttn", self.output_shape[1:], self.get_num_parameters(),
                f"{self.num_heads}h"]


class RotaryPositionalEncoding(BaseLayer):
    def __init__(self, d_model, max_len=5000):
        super().__init__()
        self.d_model = d_model
        self.max_len = max_len
        self.layer_type = "RotaryPositionalEncoding"
    ...
    def forward(self, inputs, training=True):
        ...
        cos = self._cos[:L]
        sin = self._sin[:L]
        return self._rotate(inputs, cos, sin)

    def backward(self, grads, learning_rate):
        L = grads.shape[1]
        cos = self._cos[:L]
        sin = self._sin[:L]
        return self._rotate(grads, cos, -sin)

    def get_num_parameters(self):
        return 0

    def summary(self):
        return ["RoPE", self.output_shape[1:], 0, ""]
